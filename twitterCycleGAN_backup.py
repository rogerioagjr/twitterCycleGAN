...
class Transformer(nn.Module):
    ...
    def forward(self, src):
        ...
        output_embeddings = self.transformer_encoder(src_positional_embeddings, self.src_mask)
...
def prepare_user_data(user):
    ...
    #with open('data/' + user + '/train.tsv', 'w') as train_file:
    ...
    #with open('data/' + user + '/val.tsv', 'w') as val_file:
    #    val_file.write('\ttext\n')
    #    for tweet in data_val:
    #        if len(tweet) > 0:
    #            val_file.write(tweet)
    #            val_file.write('\n')

    #with open('data/' + user + '/test.tsv', 'w') as test_file:
